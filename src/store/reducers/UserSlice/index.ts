import { createSlice } from '@reduxjs/toolkit'
import type { PayloadAction } from '@reduxjs/toolkit'
import {UserState} from './types'
import { db } from '../../../firebase';
import { doc, setDoc, collection, addDoc, getDocs, updateDoc, arrayUnion} from "firebase/firestore"; 

const localStore = localStorage.getItem('user')
const userStore = (typeof localStore === 'string')? JSON.parse(localStore) : null

const initialState: UserState = {
	currentUser: userStore,
	// settings: {
		baseCurrency: "USD",
		colorThema: 'white',
		whiteThemaId: 'standart',
		darkThemaId: 'standart',
	// }
}

export const userSlice = createSlice({
	name: 'user',
	initialState,
	reducers: {
		setUser(state, action){
			const {displayName, email, uid} = action.payload
			localStorage.setItem('user', JSON.stringify({displayName, email, uid}))
			state.currentUser = {displayName, email, uid}
			
			//// Old, without secyrity
			// localStorage.setItem('user', JSON.stringify(action.payload.user))
			// state.currentUser = action.payload
		},
		settingsChange(state){
			// console.log(111);
			
			if (state.currentUser !== null && state.currentUser.uid !== null) {
				const uid = state.currentUser.uid;
				(async () => {
					// console.log(222);
					//✅✅✅
					// await setDoc(doc(db, "users", uid), {
					// 	displayName: state.currentUser?.displayName,
					// 	email: state.currentUser?.email,
					// 	uid: state.currentUser?.uid,
					// 	baseCurrency: state.baseCurrency,
					// 	colorThema: state.colorThema,
					// 	whiteThemaId: state.whiteThemaId,
					// 	darkThemaId: state.darkThemaId,
					// });

					//✅✅✅
						// await setDoc(doc(db, "currency", "2022-08-30_13:30:00"), {
						// //   "base": "USD",
						// //   "results": {
						// //     "AED": 3.6717,
						// //     "AFN": 88.53372,
						// //     "ALL": 116.51725,
						// //     "AMD": 404.11779,
						// //     "ANG": 1.78566,
						// //     "AOA": 428.27639,
						// //     "ARS": 138.54055,
						// //     "AUD": 1.44655,
						// //     "AWG": 1.7979,
						// //     "AZN": 1.69885,
						// //     "BAM": 1.95236,
						// //     "BBD": 1.9988,
						// //     "BDT": 95.18297,
						// //     "BGN": 1.95524,
						// //     "BHD": 0.37572,
						// //     "BIF": 2041.2291,
						// //     "BMD": 0.99892,
						// //     "BND": 1.39458,
						// //     "BOB": 6.9205,
						// //     "BRL": 5.0503,
						// //     "BSD": 1.0009,
						// //     "BTN": 79.95253,
						// //     "BWP": 12.76676,
						// //     "BZD": 2.01348,
						// //     "CAD": 1.30245,
						// //     "CDF": 1997.99361,
						// //     "CHF": 0.97114,
						// //     "CLF": 0.02564,
						// //     "CLP": 874.60513,
						// //     "CNH": 6.90832,
						// //     "CNY": 6.90194,
						// //     "COP": 4360.22745,
						// //     "CUP": 24.00961,
						// //     "CVE": 110.73886,
						// //     "CZK": 24.4745,
						// //     "DJF": 178.87745,
						// //     "DKK": 7.41233,
						// //     "DOP": 52.93897,
						// //     "DZD": 141.00883,
						// //     "EGP": 19.19062,
						// //     "ERN": 15.14352,
						// //     "ETB": 52.79366,
						// //     "EUR": 0.99672,
						// //     "FJD": 2.2108,
						// //     "FKP": 0.85378,
						// //     "GBP": 0.85378,
						// //     "GEL": 2.8652,
						// //     "GHS": 9.798,
						// //     "GIP": 0.85378,
						// //     "GMD": 55.68644,
						// //     "GNF": 8600.48153,
						// //     "GTQ": 7.71818,
						// //     "GYD": 209.08509,
						// //     "HKD": 7.84927,
						// //     "HNL": 24.43668,
						// //     "HRK": 7.44067,
						// //     "HTG": 119.46536,
						// //     "HUF": 401.51546,
						// //     "IDR": 14864.67565,
						// //     "ILS": 3.31096,
						// //     "INR": 79.5294,
						// //     "IQD": 1456.74743,
						// //     "IRR": 41999.47066,
						// //     "ISK": 141.56976,
						// //     "JMD": 148.21474,
						// //     "JOD": 0.70719,
						// //     "JPY": 138.32198,
						// //     "KES": 119.67795,
						// //     "KGS": 80.828,
						// //     "KHR": 4093.09632,
						// //     "KMF": 492.39708,
						// //     "KPW": 899.91974,
						// //     "KRW": 1345.29481,
						// //     "KWD": 0.30786,
						// //     "KYD": 0.81895,
						// //     "KZT": 473.02819,
						// //     "LAK": 15385.4823,
						// //     "LBP": 1508.34144,
						// //     "LKR": 362.52156,
						// //     "LRD": 153.70285,
						// //     "LSL": 16.85604,
						// //     "LYD": 4.93451,
						// //     "MAD": 10.53421,
						// //     "MDL": 19.32773,
						// //     "MGA": 4147.08092,
						// //     "MKD": 61.24763,
						// //     "MMK": 2094.16355,
						// //     "MNT": 3205.47103,
						// //     "MOP": 8.07239,
						// //     "MRU": 37.72006,
						// //     "MUR": 44.52444,
						// //     "MVR": 15.43884,
						// //     "MWK": 1017.46837,
						// //     "MXN": 19.99035,
						// //     "MYR": 4.48089,
						// //     "MZN": 63.63173,
						// //     "NAD": 16.79286,
						// //     "NGN": 418.86857,
						// //     "NOK": 9.74011,
						// //     "NPR": 127.51908,
						// //     "NZD": 1.62226,
						// //     "OMR": 0.38389,
						// //     "PAB": 0.99892,
						// //     "PEN": 3.82757,
						// //     "PGK": 3.52354,
						// //     "PHP": 56.23602,
						// //     "PKR": 220.34903,
						// //     "PLN": 4.7104,
						// //     "PYG": 6896.61561,
						// //     "QAR": 3.63784,
						// //     "RON": 4.84862,
						// //     "RSD": 116.61117,
						// //     "RUB": 61.19188,
						// //     "RWF": 1031.82897,
						// //     "SAR": 3.75337,
						// //     "SCR": 14.34865,
						// //     "SDG": 565.16145,
						// //     "SEK": 10.63171,
						// //     "SGD": 1.39487,
						// //     "SHP": 0.85378,
						// //     "SLL": 14103.01726,
						// //     "SOS": 568.97358,
						// //     "SRD": 24.62912,
						// //     "SYP": 2511.02232,
						// //     "SZL": 16.84195,
						// //     "THB": 36.42155,
						// //     "TJS": 10.22805,
						// //     "TMT": 3.49483,
						// //     "TND": 3.19509,
						// //     "TOP": 2.34408,
						// //     "TRY": 18.17018,
						// //     "TTD": 6.71969,
						// //     "TWD": 30.46651,
						// //     "TZS": 2332.06974,
						// //     "UAH": 36.90274,
						// //     "UGX": 3804.20231,
						// //     "USD": 1,
						// //     "UYU": 40.44277,
						// //     "UZS": 10938.22038,
						// //     "VND": 23440.56828,
						// //     "VUV": 117.58448,
						// //     "WST": 2.69209,
						// //     "XAF": 653.65931,
						// //     "XCD": 2.69873,
						// //     "XDR": 0.76666,
						// //     "XOF": 653.65931,
						// //     "XPF": 118.82161,
						// //     "YER": 249.65849,
						// //     "ZAR": 16.85544,
						// //     "ZMW": 16.04025
						// //   },
						// //   "updated": "2022-08-30 13:30:00",
						// });


					//✅✅✅
					// const cityRef = doc(db, "users", uid);
					// // setDoc(cityRef, data, { merge: true });
					
					// setDoc(cityRef, {"Cards && Wallets":{
					// 	"#id":{
					// 		name: "TestWallet2",
					// 		currency: ["EUR", "UAN"],
					// 		type: 'Wallet',
					// 		values: [200, 500]
					// 	}
					// 	}}, { merge: true });
					




				// 	const washingtonRef = doc(db, "users", uid);

				// 	await updateDoc(washingtonRef, {
				// 		// 'Cards && Wallets': arrayUnion("greater_virginia")
				// 		// 'Cards && Wallets': increment(50)
				//   });

				})()
			}
		},
		singOut(state){
			localStorage.setItem('user', JSON.stringify(null))
			state.currentUser = null
		}

	}
})

export default userSlice.reducer
